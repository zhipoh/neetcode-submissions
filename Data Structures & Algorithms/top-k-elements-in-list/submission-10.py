class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        # build the frequency map for number counter
        count = defaultdict(int)
        for num in nums:
            count[num] = count.get(num,0) + 1

        # create frequency table, we add the number based on the frequency (Max frequency is length of nums, min frequency is 1)
        freq = [[] for i in range(len(nums) + 1)]
        for num, count in count.items():
            freq[count].append(num)

        # Get in reverse, first k item
        res = []
