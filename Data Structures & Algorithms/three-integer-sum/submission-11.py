class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        for i in range(len(numsort) - 1):
            ...
            # Important because if first value is more than zero, the remaining sum will never add up to zero
            if numsort[i] > 0:
                break

            # Important because if the value before is the same, then we will add back the same
            # i > 0 to address edge case where first value is duplicated 
